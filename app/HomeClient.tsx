...
interface HomeProps {
  /** True when the request reached us with a Supabase session. */
  signedIn: boolean;
  /** Name of the user's stored resume (`profiles.resume_filename`), or null
   * if signed-out or signed-in-with-no-stored-resume. When set, the home page
   * hides its file-upload UI and the generate flow uses the stored text. */
  storedResumeFilename: string | null;
}

export default function Home({ signedIn, storedResumeFilename }: HomeProps) {
  // Derived flag — true when we should drive the form off the stored
  // resume on the user's profile instead of asking for an upload.
  const usingStoredResume = signedIn && !!storedResumeFilename;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formState.jd.trim()) {
      setError('Please paste a job description.');
      return;
    }
    if (!usingStoredResume && !formState.resume) {
    ...
    try {
      ...
      // Only attach a file if the user has explicitly uploaded one for
      // this generation. When `usingStoredResume` is true the server
      // falls back to `profiles.resume_text` — no need to re-upload.
      if (formState.resume) {
  ...
  // Signed-in users with a stored resume don't need to attach a file —
  // the server uses the text on their profile. Anon/no-stored users
  // still need to upload one.
  const canSubmit =
    formState.jd.trim().length > 0 &&
    (usingStoredResume || !!formState.resume) &&
    !isLoading &&
    !quotaExhausted;
        ...
        <div className="relative rounded-3xl bg-slate-950/60 backdrop-blur-xl border border-white/10 p-6 md:p-8 shadow-2xl">
          <form onSubmit={handleSubmit}>
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              {/* Resume input — two modes:
                  1. usingStoredResume === true → show a compact "using
                     your saved resume" panel with a link to /account to
                     replace. No upload UI; the server uses
                     profiles.resume_text.
                  2. usingStoredResume === false → the original click +
              ...
              {usingStoredResume ? (
                <div>
                  ...
                  <div className="flex items-center justify-between gap-3 px-4 py-4 rounded-xl border border-emerald-400/30 bg-emerald-400/5">
                    <div className="flex items-center gap-3 min-w-0">
                      <svg
                        className="w-6 h-6 text-emerald-300 shrink-0"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="1.6"
                    ...
                    <Link
                      href="/account"
                      className="shrink-0 text-xs px-3 py-1.5 rounded-md bg-white/10 text-white hover:bg-white/20 transition"
                    >
                      Change in account →
                    </Link>
                  </div>
                  <p className="mt-2 text-xs text-white/40">
                    Replace it from your account page whenever your CV changes.
                  </p>
                </div>
              ) : (
              <div>
